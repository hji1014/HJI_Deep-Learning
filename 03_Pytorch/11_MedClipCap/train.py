...
class ClipCocoDataset(Dataset):
    ...
    def __init__(self, data_path: str,  prefix_length: int, gpt2_type: str = "gpt2", normalize_prefix=False):
        ...
        #self.tokenizer.pad_token = self.tokenizer.eos_token             # 어차피 pad_token을 쓰지 않음. 그래서 해줄 필요가 없다고 생각함.
        ...
        self.captions = [caption['caption'] + ' <|endoftext|>' for caption in captions_raw]
...
def train(dataset: ClipCocoDataset, model: ClipCaptionModel, args,
    ...
    for epoch in range(epochs):
        ...
        for idx, (tokens, mask, prefix) in enumerate(train_dataloader):
            ...
            """
            이 곳에 val_loss 구하는 코드 추가하면 될듯(위에서 val_data 부르는 코드 추가하고)
            """
            loss.backward()
            optimizer.step()
            scheduler.step()
...
def main():
    ...
    parser.add_argument('--only_prefix', dest='only_prefix', action='store_true')                   # True인 경우 transformer mapping network 사용(커맨드에 --only_prefix 추가 시 True로 변경됨)
